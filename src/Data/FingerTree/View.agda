...
... | nilₗ ⇑[ p ] = digitToTree rs [ _ ∙> r ⟿ r ] (λ rs′ → pure (a ◃ rs′)) ≈[ ∙≫ sym (identityˡ _) ⍮ (∙≫ ≪∙ p) ⍮ 𝓂≈ ]
...
... | nilᵣ ⇑[ p ] = digitToTree ls [ l <∙ _ ⟿ l ] (λ ls′ → pure (ls′ ▹ a)) ≈[ ℳ ↯ ↣ μ ls ∙ (ε ∙ μ a) ] ≈[ ∙≫ ≪∙ p ⍮ 𝓂≈ ]′

-- deepₗ : ∀ {a} {Σ : Set a} ⦃ _ : σ Σ ⦄ → (l : List Σ) → (m : Tree ⟪ Node Σ ⟫) → (r : Digit Σ) → μ⟨ Tree Σ ⟩≈ (μ l ∙ (μ m ∙ μ r))
-- deepₗ [] m r = deep (μ m ∙ μ r ↤ (
--   (viewₗ m [ ms <∙ _ ⟿ ms ] λ where
--     nilₗ → {!!}
--     ((μ⟨y⟩ ↤ N₂ y₁ y₂    ⇑[ yp ]) ◃ ys) → D₂ y₁ y₂ & ys & r ⇑[ ℳ ↯ ] ≈[ ≪∙ ≪∙ yp ]′
--     ((μ⟨y⟩ ↤ N₃ y₁ y₂ y₃ ⇑[ yp ]) ◃ ys) → D₃ y₁ y₂ y₃ & ys & r ⇑[ ℳ ↯ ] ≈[ ≪∙ ≪∙ yp ]′ ))) ⇑[ ℳ ↯ ]
