...
_◂_ : ∀ {a} {Σ : Set a} ⦃ _ : σ Σ ⦄ → ∀ {x xs} → μ⟨ Σ ⟩≈ x → μ⟨ Tree Σ ⟩≈ xs → μ⟨ Tree Σ ⟩≈ (x ∙ xs)
x ◂ xs = do
  a ← x ⇓[ ≪∙_ ]
  xs ⇓[ ∙≫_ ] >>= λ where
    empty → single a ⇑[ ℳ ↯ ]
    (single b) → deep ⟪ D₁ a & empty & D₁ b ⇓⟫ ⇑[ ℳ ↯ ]
    (deep ys) → pure $ deep (𝓢 (((pure ys ⟪⇓⟫[ ∙≫_ ]) ⟪>>=⟫ λ where
      (D₁ b & m & rs) → D₂ a b & m & rs ⇑[ ℳ ↯ ]
      (D₂ b c & m & rs) → D₃ a b c & m & rs ⇑[ ℳ ↯ ]
      (D₃ b c d & m & rs) → D₄ a b c d & m & rs ⇑[ ℳ ↯ ]
      (D₄ b c d e & m & rs) → (do
        m′ ← (pure ⟪ N₃ c d e ⇓⟫ ◂ pure m) ⇓[ (λ p → ∙≫ ≪∙ p) ]
        pure (D₂ a b & m′ & rs)) ≈[ ℳ ↯ ]
       )))
-- a ◂ empty = single a ↦ ℳ ↯
...
-- listToTree (x ∷ xs) | ys ↦ fib = [ ∙≫ fib ]≈ (x ◂ ys)
